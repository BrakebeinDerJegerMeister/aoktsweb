...
export function ScenarioStructs(ext, version) {
    ...
    function UncompressedHeader() {
        let use1 = { "use": _ => version < 1.40 }
        let use2 = { "use": _ => ext == "aoe2scenario" }
        let use3 = { "use": _ => use2.use() && version >= 1.35 }

        this.version = ascii(4, showMe);
        ...
        this.usedSets = arrayOf(u32(), _ => this.usedSetsCount.value, use2);
    ...
    function ScenarioHeader() {
        let afterRead1 = { "afterRead": _ => currentVersion = this.version.value }
        let use1 = { "use": _ => currentVersion >= 1.18 }
        ...
        this.missionItem = arrayOf(arrayData(30), _ => this.missionItemsCounter.value);
    ...
    function Messages() {
        let use1 = { "use": _ => currentVersion >= 1.18 }
        let use2 = { "use": _ => currentVersion >= 1.22 }
    ...
    function CompressedSection() {
        this.scenarioHeader = section(ScenarioHeader);
        this.messages = section(Messages);
        this.cinematics = section(Cinematics);
        this.bitmap = section(Bitmap);
        this.playerData2 = section(PlayerData2);
        this.globalVictory = section(GlobalVictory);
        this.diplomacy = section(Diplomacy);
        this.disables = section(Disables);
        this.gameMap = section(GameMap);
        this.unitsSection = section(UnitsSection);
        this.playerData3 = section(PlayerData3);
        this.triggers = section(Triggers);
        this.includedFiles = section(IncludedFiles);
        this.unknown = section(Unknown);
    }
